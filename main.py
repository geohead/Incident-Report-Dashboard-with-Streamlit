...
# the code below fetches from an api endpoint in json format

url = 'http://127.0.0.1:8000/api/incidentreporter/'
...
# convert the geojson response to a pandas dataframe by dropping the geometry column.
gdf = gpd.read_file(data).drop("geometry", axis=1)
